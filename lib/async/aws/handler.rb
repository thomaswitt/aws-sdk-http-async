  ...
  module Aws
    class Handler < Seahorse::Client::Handler
      ...
      def call(context)
        ...
        return fallback_handler(context) if force_fallback? || !async_context?
      ...
      def transmit_inner(config, req, resp)
        ...
        client = cache.client_for(req.endpoint, config)
      ...
      def body_size(body, headers)
        content_length = headers['content-length'] || headers['Content-Length'] || headers['CONTENT_LENGTH']
        return content_length.to_i if content_length && !content_length.to_s.empty?
      ...
      def delegate_event_stream(context)
        ...
        if context.client.respond_to?(:connection)
          return Seahorse::Client::H2::Handler.new(nil).call(context)
        end
      ...
      def read_with_timeout(response, config)
        ...
        body = response.body
        return nil if body.nil?
        return body.read if timeout.nil? || timeout <= 0

        Async::Task.current.with_timeout(timeout, Async::TimeoutError) do
          begin
            body.read(StreamingBody::CHUNK_SIZE)
          rescue ArgumentError
            body.read
      ...
      def call_with_timeout(client, request, config)
        ...
        body = request.body
        return client.call(request) unless body.nil? || body.is_a?(String)
